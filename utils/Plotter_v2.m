...
ext = '.png';
folder = 'figs/MainSimv1/SADC_PP5/';
...
%% Estimate of Normalized Attitude Constraint
figType = 'constraint_estimate';
if strcmp(AttType,'quaternion')
    ylab = '$\hat{q}^{T}\hat{q}-1$';
    titl = '$\hat{q}^{T}\hat{q}-1$ vs $t$';
elseif strcmp(AttType,'DCM')
    ylab = '$det(\hat{C_{ba}})-1$';
    titl = '$det(\hat{C_{ba}})-1$ vs $t$';    
end

figure(10)
plot(tout,constrainthat), xlabel('$t$ (s)','fontsize',14,'interpreter','latex');
ylabel(ylab,'fontsize',14,'interpreter','latex'); 
title(titl,'fontsize',16,'interpreter','latex');
...
%% Euler Angle Error from Inertial Navigation (3-2-1 Sequence)
figType = 'euler_err_IN';
figure(11)
subplot(3,1,1)
plot(tout,eulerEstErr_IN(1,:)*rad2deg), xlabel('$t$ (s)','fontsize',14,'interpreter','latex');
ylabel('$\phi^{IN}_{err}$ (deg)','fontsize',14,'interpreter','latex'); 
title('$\phi^{IN}_{err}$ vs $t$','fontsize',16,'interpreter','latex');

subplot(3,1,2)
plot(tout,eulerEstErr_IN(2,:)*rad2deg), xlabel('$t$ (s)','fontsize',14,'interpreter','latex');
ylabel('$\theta^{IN}_{err}$ (deg)','fontsize',14,'interpreter','latex'); 
title('$\theta^{IN}_{err}$ vs $t$','fontsize',16,'interpreter','latex');

subplot(3,1,3)
plot(tout,eulerEstErr_IN(3,:)*rad2deg), xlabel('$t$ (s)','fontsize',14,'interpreter','latex');
ylabel('$\psi^{IN}_{err}$ (deg)','fontsize',14,'interpreter','latex'); 
title('$\psi^{IN}_{err}$ vs $t$','fontsize',16,'interpreter','latex');

savePath = strcat(folder,figType,ext);
saveas(gcf,savePath);

%% Euler Angle Error from TRIAD Algorithm (3-2-1 Sequence)
figType = 'euler_err_TRIAD';
figure(12)
subplot(3,1,1)
plot(tout,eulerEstErr_TRIAD(1,:)*rad2deg), xlabel('$t$ (s)','fontsize',14,'interpreter','latex');
ylabel('$\phi^{TRIAD}_{err}$ (deg)','fontsize',14,'interpreter','latex'); 
title('$\phi^{TRIAD}_{err}$ vs $t$','fontsize',16,'interpreter','latex');

subplot(3,1,2)
plot(tout,eulerEstErr_TRIAD(2,:)*rad2deg), xlabel('$t$ (s)','fontsize',14,'interpreter','latex');
ylabel('$\theta^{TRIAD}_{err}$ (deg)','fontsize',14,'interpreter','latex'); 
title('$\theta^{TRIAD}_{err}$ vs $t$','fontsize',16,'interpreter','latex');

subplot(3,1,3)
plot(tout,eulerEstErr_TRIAD(3,:)*rad2deg), xlabel('$t$ (s)','fontsize',14,'interpreter','latex');
ylabel('$\psi^{TRIAD}_{err}$ (deg)','fontsize',14,'interpreter','latex'); 
title('$\psi^{TRIAD}_{err}$ vs $t$','fontsize',16,'interpreter','latex');
...
AttitudeAnimEst(savePath,q,q_IN,q_TRIAD,l,w,d,tout,10);
